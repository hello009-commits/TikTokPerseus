...
rapidapi_server_domain = "tiktokperseus4.p.rapidapi.com"
rapidapi_server_host = f"https://{rapidapi_server_domain}"


def get_api_common_params(endpoint, params=True):
    if len(rapidapi_key) > 0:
        if params:
            url = f"{rapidapi_server_host}/{endpoint}/Android"
        else:
        ...
        raise Exception("Please fill in your rapidapi key from https://rapidapi.com/reverse4free4/api/tiktokperseus4.")
...
def encrypt_get_setting(dev_info):
    url, payload, headers = get_api_common_params("get_mscc_body", False)
    payload = json.dumps(payload | {
        "dev_info": dev_info
    })
    printf(payload)
